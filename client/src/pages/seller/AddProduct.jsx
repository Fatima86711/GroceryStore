        ...
        <div className="no-scrollbar flex-1 h-[95vh] overflow-y-scroll flex flex-col justify-between">
            <form onSubmit={onSubmitHandler} className="md:p-10 p-4 space-y-5 max-w-lg">
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                        {Array(4).fill('').map((_, index) => (
                            <label key={index} htmlFor={`image${index}`}>
                                ...
                                <img src={file[index]? URL.createObjectURL(file[index]): assets.upload_area} className='max-w-24 cursor-pointer' alt="" />
                                 </label>
                        ))}
                    </div>
                </div>
                <div className="flex flex-col gap-1 max-w-md">
                    ...
                    <input value = {name} onChange={(e)=>{setName(e.target.value)}} id="product-name" type="text" placeholder="Type here" className="outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500/40" required />
                </div>
                <div className="flex flex-col gap-1 max-w-md">
                ...
                <div className="flex items-center gap-5 flex-wrap">
                    ...
                    <div className="flex-1 flex flex-col gap-1 w-32">
                        ...
                        <input id="offer-price"  onChange={(e) => setOfferPrice(e.target.value)} value={offerPrice} type="number" placeholder="0" className="outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500/40" required />
                    </div>
                </div>

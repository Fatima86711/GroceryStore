...
const Login = () => {
    const {showUserLogin,setShowUserLogin,setUser, axios, navigate } = useAppContent();
  ...
  return (
    <div onClick={()=>setShowUserLogin(false)} className='fixed  top-0 bottom-0 left-0 right-0 z-200 flex items-center text-sm text-gray-600 bg-black/50' >
            ...
            {state === "register" ? (
                <p>
                    Already have account? <span onClick={() => {setState("login"); 
                         setName("");}} className="text-primary cursor-pointer">click here</span>
                </p>
            ) : (
                <p>
